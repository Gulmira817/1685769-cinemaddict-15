...
const createFilmDetailsTemplate = (data, newComment) => {
  const { comments, movieInfo, userDetails, isDisabled, deletingId } = data;

  const watchlistName = userDetails.isWhatchList ? 'film-details__control-button--active' : '';
  const watchedName = userDetails.isHistory ? 'film-details__control-button--active' : '';
  const favoritesName = userDetails.isFavorite ? 'film-details__control-button--active' : '';
...
export default class FilmsDetails extends SmartView {
  constructor(film, changeData, commentsModel, api) {
    ...
    this._data = FilmsDetails.parseFilmToData(film, []);
    this._api = api;
  ...
  getTemplate() {
    return createFilmDetailsTemplate(this._data, this._newComment);
  }
  ...
  _setInnerHandlers() {
    ...
    this.getElement().querySelector('.film-details__comment-input').addEventListener('input', this._commentTextChangeHandler);
    ...
    if (evt.key === 'Enter') {
      ...
      if (this._newComment.emoji && this._newComment.comment) {
        ...
        this._api.addComment(this._data, this._newComment)
          .then((response) => {
            this._data = FilmsDetails.parseDataToFilm(this._data);
            this._changeData(
  ...
  restoreHandlers() {
    this._setInnerHandlers();
    this.setFormSubmitHandler(this._callback.click);
    this.setHistoryClickHandler(this._callback.alreadyWatchedClick);
    this.setWhatchClickHandler(this._callback.watchlistClick);
